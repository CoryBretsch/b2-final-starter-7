...
RSpec.describe "coupon show" do
  before :each do
    ...
    @transaction6 = Transaction.create!(credit_card_number: 879799, result: 0, invoice_id: @invoice_7.id)
    @transaction7 = Transaction.create!(credit_card_number: 203942, result: 0, invoice_id: @invoice_2.id)
  end

  describe "User Story 3" do 
    it "can display coupons show page with all attributes and total times used" do 
      ...
      expect(page).to have_content("Discount: #{@coupon1.category} is #{@coupon1.amount}")
      expect(page).to have_content("Active: #{@coupon1.active}")
